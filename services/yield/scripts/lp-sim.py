...
if args.md:
    L = []
    L.append("# MODEL-NUMBERS — the Base module's yield model, generated " + out["generatedAt"])
    L.append("")
    ...
    if out["verdict"]["clears"]:
        L.append("**Beats the borrow on both models:** " + ", ".join(f"{c['pool']} / {c['setting']} (lpNet {c['lpNetPct']:+.2f}%, mcLpNet {c['mcLpNetPct']:+.2f}%)" for c in out["verdict"]["clears"]))
    else:
        L.append("**No pool × setting beats the borrow on both models.** Every cell is shown and can be opened after the acknowledgment (BUILD-PLAN D4/D5); hold-USDC and spot stand beside it.")
    L.append("")
    def fail_txt(c):
