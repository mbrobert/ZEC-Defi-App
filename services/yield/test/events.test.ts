...
import { DATA_WORDS, decodeEngineLog, MalformedLogError, TOPIC_COUNTS, TOPICS } from "../src/engine/events.js";
...
test("STRICT decoding: a short/empty PositionWithdrawn is malformed, never zero amounts (audit Lens F)", () => {
  const short = { ...LOGS[2], data: "0x" as `0x${string}` };
  assert.throws(() => decodeEngineLog(short), MalformedLogError);
  const oneWord = { ...LOGS[2], data: ("0x" + "00".repeat(32)) as `0x${string}` };
  assert.throws(() => decodeEngineLog(oneWord), (e: unknown) => e instanceof MalformedLogError && e.kind === "PositionWithdrawn");
  const tooLong = { ...LOGS[2], data: (LOGS[2].data + "00".repeat(32)) as `0x${string}` };
  assert.throws(() => decodeEngineLog(tooLong), MalformedLogError);
});

test("STRICT decoding: wrong topic count or a malformed topic word is malformed for every kind", () => {
  for (const [i, log] of LOGS.entries()) {
    const fewer = { ...log, topics: log.topics.slice(0, -1) };
    assert.throws(() => decodeEngineLog(fewer), MalformedLogError, `fewer topics #${i}`);
    const extra = { ...log, topics: [...log.topics, log.topics[1]!] };
    assert.throws(() => decodeEngineLog(extra), MalformedLogError, `extra topics #${i}`);
    const badWord = { ...log, topics: log.topics.map((t, j) => (j === 1 ? ("0x1234" as `0x${string}`) : t)) };
    assert.throws(() => decodeEngineLog(badWord), MalformedLogError, `bad topic word #${i}`);
  }
});

test("STRICT decoding: every captured live log matches its declared shape exactly", () => {
  for (const log of LOGS) {
    const kind = (Object.keys(TOPICS) as (keyof typeof TOPICS)[]).find((k) => TOPICS[k] === log.topics[0])!;
    assert.equal(log.topics.length, TOPIC_COUNTS[kind]);
    assert.equal((log.data.length - 2) / 64, DATA_WORDS[kind]);
  }
});
