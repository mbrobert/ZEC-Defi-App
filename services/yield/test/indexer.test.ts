...
test("fillEntryFlows (raw-receipt path): nets deposit inflows minus same-tx refunds", async () => {
  ...
  try {
    ...
    // Independently computed from the captured receipt — inflows and
    // refunds are kept SEPARATE (v2): WETH 0x572e83490cdbc00 in, 0x11e back;
    // USDC 0x3b193628 in, 0x859b19 back.
    assert.equal(created.entryFlows!["0x4200000000000000000000000000000000000006"], "392631430000000000");
    assert.equal(created.entryFlows!["0x833589fcd6edb6e08f4c7c32d4f71b54bda02913"], "991508008");
    assert.equal(created.entryRefunds!["0x4200000000000000000000000000000000000006"], "286");
    assert.equal(created.entryRefunds!["0x833589fcd6edb6e08f4c7c32d4f71b54bda02913"], "8755993");
    assert.equal(created.entryTxPositions, 1);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("scan: a malformed indexed log is counted in state and skipped — never booked as zeros", async () => {
  const dir = tempDir();
  try {
    const store = new EventStore(dir, VAULT);
    const broken: RawLog = { ...LOGS[2], data: "0x", logIndex: 9999 }; // short PositionWithdrawn
    const rpc = new StubRpc(50_534_000, [...LOGS, broken]);
    const indexer = new EngineIndexer(rpc, store, VAULT, 10_000);
    const r = await indexer.scan();
    assert.equal(r.events, LOGS.length);
    assert.equal(r.malformed, 1);
    assert.equal(store.readState()!.malformedLogs, 1);
    assert.ok(!store.readAll().some((e) => e.logIndex === 9999));
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});

test("fillEntryFlows: a deposit tx that created TWO positions is marked (entryTxPositions=2) for refusal", async () => {
  const dir = tempDir();
  try {
    const store = new EventStore(dir, VAULT);
    // Duplicate the first PositionCreated under another tokenId in the SAME tx.
    const twin: RawLog = {
      ...LOGS[0],
      topics: [LOGS[0].topics[0], ("0x" + "1".padStart(64, "0")) as Hex, LOGS[0].topics[2], LOGS[0].topics[3]],
      logIndex: 249,
    };
    const rpc = new StubRpc(50_534_000, [...LOGS, twin]);
    const indexer = new EngineIndexer(rpc, store, VAULT, 10_000);
    await indexer.scan();
    await indexer.fillEntryFlows();
    const created = store
      .readAll()
      .filter((e): e is PositionCreatedEvent => e.kind === "PositionCreated" && e.transactionHash === DEPOSIT_RECEIPT.tx);
    assert.equal(created.length, 2);
    for (const c of created) assert.equal(c.entryTxPositions, 2);
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
