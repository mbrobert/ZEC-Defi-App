...
export function describeGrant(grant: KeeperGrantRead | null, opts: { keeperConfigured: boolean; nowSeconds: number }): KeeperStatus {
  ...
  return {
    ...
    plain:
      ...
      `${daysLeft <= GRANT_EXPIRY_WARN_DAYS ? ", which is very soon; re-grant it to keep the protection" : ""}. It cannot open, borrow, sweep or move a token on its own, and you can revoke it at any time.`,
