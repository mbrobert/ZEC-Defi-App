...
At the USDC variable borrow rate of **4.5174 %** (Aave v3 Base
`PoolDataProvider.getReserveData(USDC)`, read live 2026-09-12 19:31 UTC at
block 51,226,072 together with every Aerodrome gauge word,
`docs/VERIFIED-BASE-FACTS.md` Addendum 12) **no pool × setting clears the
yield gate**: 0 of 27 cells in `docs/MODEL-NUMBERS-2026-09-12.md` (generated
by `services/yield/scripts/lp-sim.py` from that sample — `npm run model`
reads every input from the sample file — and pinned cell-by-cell by
...
"sheltered" width, nets **−10.92 %/yr** on the LP slice after the engine's
15 % fee, Oilskin's 10 % performance fee and the impermanent-loss drag at the
recorded volatility (−5.29 % on the 2026-08-31 words: the gauge pays a
marginal staker less than half of what it did); it would need **4.56×**
today's net emissions to clear. Every priced cell's LP slice is negative, so
no borrow rate would open the menu at today's emissions. The cbZEC/USDC gauge
carries its first emissions vote (≈ 617 AERO a day to 2026-09-17) — 1 to 17 %
gross across the widths — and is still refused: below the borrow at the two
wider widths, no calibrated σ at the narrowest.
...
At the boundary where the closed form used to flip, it is 0.1–32 points
optimistic; six of the seven cells it would have offered at their own
published break-even are refused as `within_model_uncertainty`
(`docs/MODEL-NUMBERS-2026-09-12.md`; one cell, WETH/cbBTC at the working
width, also breaches the closed form's own tolerance at today's emissions —
`docs/RISKS.md` §14).

Gauge-emission inputs are the 2026-09-12 words (block 51,226,072); σ is the
2026-08-31 realized volatility. Re-run `npm run backfill -- sample` (with
`BASE_RPC_URL` set to a Base RPC that serves the batched read and
`GECKO_MIN_INTERVAL_MS=15000` for GeckoTerminal) and `npm run model` in
`services/yield` before believing any number.
