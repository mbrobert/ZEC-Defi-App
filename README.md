...
|---|---|---|
| `contracts/` | Foundry — `OilskinAccount` + factory, `StrategyRouter`, `AaveV3Venue`, `SnuggleLpVenue`, `SlipstreamLpVenue` + `SlipstreamPoolSwapAdapter` (cbZEC/USDC held directly on the second Slipstream deployment, 2026-09-11), `CollateralRegistry`, `AerodromeSwapAdapter`, `MorphoBlueVenue` (built over the two verified Base markets; not the registry's venue until propose → timelock → accept), `PythOracleAdapter` (v1.1, unused) | 363 passed / 0 failed / 12 skipped (2026-09-11; the 12 fork tests need `FORK_URL` — 10 of them run against Base on 2026-09-10: 9 pass / 1 fail, `docs/TESTING.md`), 26 suites |
