...
for (const id of Object.keys(model.results)) {
  ...
  if (st.epochActive !== epochActive && /note:"/.test(trail)) {
    const newNote = epochActive
      ? `gauge epoch active at the ${sampledAt.slice(0, 10)} sample${sigmaOf(id) === null ? " — no σ calibrated, gate refuses" : ""}`
...
const blockText = `${START} /tmp/build/MODEL-NUMBERS.md (regenerated
   ...
   calibrated σ — the gate REFUSES the rest with no_volatility_input).
   ...
   \`mc\` carries the Monte-Carlo calibration the gate now decides on alongside
   the closed form; \`served\` and \`userNetPinned\` are pins the page RECOMPUTES
   and the suites assert — no verdict on this page is read from them.
...
${keep}
  /* The gate's own bounds (services/yield/src/gate.ts). Refusals, never clamps. */
${bounds}
  pools: [
${poolLines.join("\n")}
  ],
...
${mcCells.join("\n")}
  ...
  /* [poolId, widthBps, lpNetPct|null, reason] — the served verdict at each
     preset width. lpNet is null wherever the gate refuses BEFORE it prices the
     cell, exactly as /v1/gate leaves it. "ok" would mean offered. */
  served: [
${servedLines.join("\n")}
  ],
  /* [poolId, widthBps, collateral, ltvPct, userNetPct] — a sample of the served
     user-net rows; a slot whose cell the gate refuses before pricing has no row. */
  userNetPinned: [
${pinnedLines.join("\n")}
  ],
