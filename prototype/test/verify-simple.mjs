...
{ const cr = await o("CHAIN_READ"); check(`facts: the page's default gate rate is the ledger's USDC borrow (${cr.aaveReserves.USDC.borrowAprPct}%, read ${cr.readAt} at block ${cr.readBlock})`, near((await o("S")).borrowPct, cr.aaveReserves.USDC.borrowAprPct, 1e-9) && cr.readBlock === 51226072); }
...
check("ltv: stops are 30/40/50 for both live assets (the gate's user-net rows still price them)", JSON.stringify(await o("ltvStopsFor", "cbBTC")) === "[30,40,50]" && JSON.stringify(await o("ltvStopsFor", "WETH")) === "[30,40,50]");
check("slider: the lowest HF offered is 1.56 on cbBTC and 1.66 on WETH — Oilskin's 50 % cap binds on both, named (shared offeredLtvBounds)", JSON.stringify(await o("offeredBounds", "cbBTC")) === '{"maxLtvBps":5000,"minHf":1.56,"binding":"product_ltv_cap"}' && JSON.stringify(await o("offeredBounds", "WETH")) === '{"maxLtvBps":5000,"minHf":1.66,"binding":"product_ltv_cap"}');
check("ladder: ladderFor derives the rungs from the entry HF — 1.55 is the floor's table, 1.30 → 1.27 / 1.19 / 1.11 / 1.05, 1.95 → 1.86 / 1.61 / 1.34 / 1.09 with hysteresis 0.09", JSON.stringify((await o("ladderFor", 1.55)).map(r => r.hf)) === "[1.5,1.35,1.2,1.05]" && JSON.stringify((await o("ladderFor", 1.3)).map(r => r.hf)) === "[1.27,1.19,1.11,1.05]" && JSON.stringify((await o("ladderFor", 1.95)).map(r => r.hf)) === "[1.86,1.61,1.34,1.09]" && await o("hysteresisFor", 1.95) === 0.09 && (await o("ladOf", null)).derived === false);
...
/* ── 2. the yield gate reproduces MODEL-NUMBERS exactly ── */
{
  ...
  const fc = await page.evaluate(() => { const oil = window.__oil; const B = oil.MODEL.borrowPctAtGeneration; const out = {};
    const weth = oil.poolById("aero-usdc-weth-5"); const g = oil.gate(weth, B, 4500), f = oil.forecast(weth, B, 4500, 1, { collateral: "cbBTC" });
    out.wethGate = g.reason; out.wethGateNet = g.net; out.weth = { priced: f.priced, allowed: f.allowed, net: f.net, mc: f.mcNet, gap: f.gap, clears: f.clears, userNet: f.userNet.length, why: oil.forecastWhy(f) };
    ...
    return out; });
  check(`forecast: WETH/USDC sheltered, which the gate stops at ${fc.wethGate} with no number, is PRICED by the forecast — both forms, the gap, a user-net ladder — and allowed`, fc.wethGate === "emissions_below_borrow" && !Number.isFinite(fc.wethGateNet) && fc.weth.priced && fc.weth.allowed && Number.isFinite(fc.weth.net) && Number.isFinite(fc.weth.mc) && Number.isFinite(fc.weth.gap) && fc.weth.clears.both === false && fc.weth.userNet === 3 && /below the/.test(fc.weth.why), JSON.stringify(fc.weth).slice(0, 240));
...
check("collateral: switching to WETH recomputes the slider's stop from LT 83% — lowest offered 1.66, Oilskin's 50 % cap named, the default pulled up to it, both marks under it and disabled with the reason", /lowest offered 1\.66 · Oilskin's 50% cap on any borrow/.test(await page.textContent("#hfMin")) && /Entry health factor 1\.66 → borrow 50\.00%/.test(await page.textContent("#riskHint")) && (await page.$$eval('#riskMarks [data-mark]', e => e.length === 2 && e.every(x => x.disabled && /not offered today/.test(x.textContent)))));
await page.click('#assetSeg [data-asset="cbBTC"]');
check("collateral: back to cbBTC — lowest offered 1.56 (0.78 ÷ 0.50), the default is that minimum, the Expert mark says why it is not offered", /lowest offered 1\.56/.test(await page.textContent("#hfMin")) && /Entry health factor 1\.56 → borrow 50\.00%/.test(await page.textContent("#riskHint")) && (await page.$eval('#riskMarks [data-mark="expert"]', e => e.disabled && /under the lowest health factor offered for cbBTC today, 1\.56/.test(e.title))));
...
check("setting: a typed borrow drives the health factor back (1,000 USDC on 0.05 cbBTC → HF = 0.05 × price × 0.78 ÷ 1,000); one above the offered maximum, an HF under it, and NaN are all pulled to / held at 1.56", await page.evaluate(() => { const o = window.__oil; o.dispatch({ type: "setAmount", amount: 0.05 }); o.dispatch({ type: "setBorrow", usdc: 1000 }); const a = o.S.sel.hf; o.dispatch({ type: "setBorrow", usdc: 1e9 }); const b = o.S.sel.hf; o.dispatch({ type: "setHf", hf: 1.2 }); const c = o.S.sel.hf; o.dispatch({ type: "setHf", hf: NaN }); const d = o.S.sel.hf; return Math.abs(a - (0.05 * o.S.price.cbBTC * 0.78) / 1000) < 1e-9 && b === 1.56 && c === 1.56 && d === 1.56 && o.S.sel.ltv === 50; }));
check("setting: the acknowledgment under the Sheltered mark is unreachable on Base today (the slider stops at 1.56) — the row is hidden and needsHfAck says so", (await page.$eval("#hfAckRow", e => e.style.display)) === "none" && await o("needsHfAck", 1.56) === false && await o("needsHfAck", 1.3) === true && /1\.30, under the Sheltered mark of 1\.55/.test(await o("hfAckText", "cbBTC", 1.3)));
...
check("ack: a sign attempt without it is refused by the reducer, and a change to the setting resets a ticked acknowledgment", await page.evaluate(() => { const o = window.__oil; o.dispatch({ type: "beginDeposit" }); const noFlow = !o.S.flow; o.dispatch({ type: "ack", on: true }); const ticked = o.S.sel.ack === true; o.dispatch({ type: "setHf", hf: 1.95 }); const reset = o.S.sel.ack === false; o.dispatch({ type: "setHf", hf: 1.56 }); return noFlow && ticked && reset; }));
...
{ const b = (await o("S")).borrowPct.toFixed(3); check(`review: every number computed — HF 1.56 with the floor and the offered minimum named, liquidation −36%, borrow at ${b}% (the page's chain-read rate), fees 15% then 10%`, /1\.56 \(floor 1\.55; lowest offered 1\.56 — Oilskin's 50% cap on any borrow\)/.test(rev) && /−36%/.test(rev) && rev.includes(`${b}%`) && /15% then 10%/.test(rev)); }
...
check("position: entry HF shown 1.56, recorded on the position as the router would (entryHf 1.56), the ladder fully armed and derived from it (1.51 / 1.36 / 1.20 / 1.05)", (await page.textContent("#pHf")) === "1.56" && (await o("S")).pos.entryHf === 1.56 && (await page.$$eval("#ladder .rung.armed", e => e.length)) === 4 && /derived from this position's recorded entry health factor 1\.56/.test(await page.textContent("#ladderNote")) && /HF < 1\.51/.test(await page.textContent("#ladder")));
...
/* ── 4. bounded accrual, sign discipline, ladder hysteresis & re-arm ── */
{
  ...
  check("ladder: on a 1.56 entry the rungs fire in order warn → repay → derisk as HF crosses 1.51 / 1.36 / 1.20 (the position's own ladder); the partial unwind lifts HF back to 1.25, the de-risk rung's re-arm level — sized like the keeper's repay, never to the entry floor", seq.warnAt >= 0 && seq.repayAt > seq.warnAt && seq.unwindAt > seq.repayAt && near(seq.hfAfterUnwind, 1.25, 0.01), JSON.stringify(seq));
...
/* ── 6. store validation and two-tab safety ── */
{
  ...
  await page.evaluate(() => { const o = window.__oil; o.dispatch({ type: "connect", provider: "coinbase" }); o.dispatch({ type: "setMult", mult: 5 }); o.dispatch({ type: "ack", on: true }); o.dispatch({ type: "beginDeposit" }); o.dispatch({ type: "flowAdvance", id: o.S.flow.id }); });
   ...
   the swap floor, the keeper's grant and the honest custody language ── */
{
  ...
  check("tester's kit: the new levers are all present (pauses, corroboration, gauge re-vote, raw batch, sandwich, grant revoke/renew/expire, the disagreement band)", ["pause:collateral","pause:borrow","pause:off","corr:off","corr:on","revote:on","revote:off","rawbatch","sandwich:on","sandwich:off","grant:revoke","grant:renew","grant:expire","mult:12.2"].every(k => html.includes(`data-tk="${k}"`)));
