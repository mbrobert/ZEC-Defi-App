...
check("ack: a sign attempt without it is refused by the reducer, and a change to the setting resets a ticked acknowledgment", await page.evaluate(() => { const o = window.__oil; o.dispatch({ type: "beginDeposit" }); const noFlow = !o.S.flow; o.dispatch({ type: "ack", on: true }); const ticked = o.S.sel.ack === true; o.dispatch({ type: "selectLtv", ltv: 40 }); const reset = o.S.sel.ack === false; o.dispatch({ type: "selectLtv", ltv: 30 }); return noFlow && ticked && reset; }));
...
{ const b = (await o("S")).borrowPct.toFixed(3); check(`review: every number computed — HF 2.60, liquidation −62%, borrow at ${b}% (the page's chain-read rate), fees 15% then 10%`, /2\.60 \(floor 1\.55\)/.test(rev) && /−62%/.test(rev) && rev.includes(`${b}%`) && /15% then 10%/.test(rev)); }
