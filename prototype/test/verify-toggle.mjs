...
if (shared) {
  ...
  check("shared: LTV_PRESET_FIXED_BPS, RANGE_WIDTH_BOUNDS, REBALANCE_DELAY_BOUNDS, RANGE_PRESETS byte-equal", eq(SHc.LTV_PRESET_FIXED_BPS, shared.LTV_PRESET_FIXED_BPS) && eq(SHc.RANGE_WIDTH_BOUNDS, shared.RANGE_WIDTH_BOUNDS) && eq(SHc.REBALANCE_DELAY_BOUNDS, shared.REBALANCE_DELAY_BOUNDS) && eq(SHc.RANGE_PRESETS, shared.RANGE_PRESETS));
  ...
  check("derived: maxOfferedLtvStopBps (whole-percent UI stop) agrees with shared: 7000 → 5600, 6000 → 4800, 7800 → 6240 → 6200", typeof shared.maxOfferedLtvStopBps === "function" && [7800, 8300, 7000, 6000, 1550].every(lt => P.maxOfferedLtvStopBps(lt) === shared.maxOfferedLtvStopBps(lt)) && P.maxOfferedLtvStopBps(7000) === 5600 && P.maxOfferedLtvStopBps(7800) === 6200);
...
/* ── 4. OIL_MODEL against MODEL-NUMBERS.md (when present) ── */
{
  ...
  if (fs.existsSync(mp)) {
    ...
    // A σ-less cell is unpriced for no σ — or, when its reading is above the plausibility ceiling, as implausible, exactly as the doc's reason says (the gate's order).
    check(`forecast: of the ${unpriced.length} cells the gate stops at, the ${withSigma.length} with a σ are PRICED by forecast() on both forms and the ${noSigma.length} without are unpriced by name — all of them allowed`, withSigma.length > 0 && withSigma.every(r => r.priced && Number.isFinite(r.net) && Number.isFinite(r.mc)) && noSigma.every(r => !r.priced && (r.unpriced === "no_volatility_input" || (r.unpriced === "emissions_implausible" && unpriced.find(u => u.id === r.id && u.w === r.w)?.reason === "emissions_implausible"))) && fcUn.every(r => r.allowed), JSON.stringify(fcUn.filter(r => r.sigma != null ? !r.priced : r.priced).slice(0, 3)));
