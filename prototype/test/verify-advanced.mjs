...
check("collateral: WETH hint names the slider's stop — lowest HF offered 1.66, Oilskin's 50 % cap binding over the 1.55 floor and Aave's 80 % — and the most you can borrow, 50.00%", /Lowest health factor offered 1\.66 — Oilskin's 50% cap on any borrow \(registry floor 1\.55; Aave's max LTV 80%; Oilskin's cap 50%\); most you can borrow 50\.00% LTV/.test(await page.textContent("#assetHint")));
{
...
check("setting: the two marks are rendered, both disabled on WETH with the reason in the title (1.55 and 1.30 sit under the 1.66 offered)", (await page.$$eval("#hfMarks [data-mark]", e => e.map(x => x.dataset.mark))).join() === "sheltered,expert" && await page.$$eval("#hfMarks [data-mark]", e => e.every(x => x.disabled && /under the lowest health factor offered for WETH today, 1\.66 — Oilskin's 50% cap/.test(x.title))));
check("setting: the slider is linear in the borrow, from 1 % LTV to the offered maximum (5000 bps), the stop named beside it", await page.$eval("#hf", e => e.min === "100" && e.max === "5000") && /lowest offered 1\.66 · Oilskin's 50% cap on any borrow/.test(await page.textContent("#hfMin")));
check("setting: the default is the Sheltered mark pulled up to the offered minimum — HF 1.66 after deposit on WETH, the floor stated, the ladder for THIS entry (1.60 / 1.42 / 1.24 / 1.06)", /HF 1\.66 after · floor 1\.55 · ladder 1\.60 \/ 1\.42 \/ 1\.24 \/ 1\.06/.test(await page.textContent("#ltvHf")) && (await page.textContent("#hfLbl")) === "1.66" && (await page.textContent("#ltvLbl")) === "50.00%");
{
  const r = await page.evaluate(() => { const o = window.__oil; const g = () => [o.S.wiz.hf, o.S.wiz.ltv]; o.dispatch({ type: "wiz", key: "hf", value: 1.2 }); const a = g(); o.dispatch({ type: "wiz", key: "hf", value: 2.5 }); const b = g(); o.dispatch({ type: "wiz", key: "hf", value: NaN }); const c = g(); o.dispatch({ type: "wiz", key: "hf", value: 0 }); const d = g(); o.dispatch({ type: "wiz", key: "borrow", value: 500 }); const e = g(); o.dispatch({ type: "wiz", key: "borrow", value: 5e6 }); const f = g(); o.dispatch({ type: "wiz", key: "hf", value: 1.66 }); return { a, b, c, d, e, f, px: o.S.price.WETH, amount: o.S.wiz.amount }; });
  check("setting: the reducer pulls an HF under the offered minimum up to it (1.2 → 1.66), takes 2.5, ignores NaN and 0, derives the LTV as ⌊LT ÷ HF⌋, and a typed borrow drives the HF back (500 USDC → 1 WETH × price × 0.83 ÷ 500; 5,000,000 → pulled to 1.66)", r.a[0] === 1.66 && r.a[1] === 50 && r.b[0] === 2.5 && r.b[1] === 33.2 && r.c[0] === 2.5 && r.d[0] === 2.5 && Math.abs(r.e[0] - (r.amount * r.px * 0.83) / 500) < 1e-9 && r.f[0] === 1.66, JSON.stringify(r));
}
...
check("pool: under what-if ×5 exactly one pool beats the borrow (cbBTC/USDC) and is the selection", /1 of 9 pools beat the borrow/.test(await page.textContent("#poolCount")) && (await o("S")).wiz.pool === "aero-cbbtc-usdc");
await page.click("#nextBtn");
{
