...
contract LoanDustRegressionTest is Fixture {
    ...
    function _supplyAave(uint256 amount) internal {
        _ownerExec(
            address(aaveVenue), abi.encodeCall(ICollateralVenue.supply, (address(cbbtc), amount))
        );
    }

    function _borrowAave(uint256 amount) internal {
        _ownerExec(
            address(aaveVenue), abi.encodeCall(ICollateralVenue.borrow, (address(usdc), amount))
        );
    }

    function _repayMax(address venue) internal returns (uint256) {
        bytes memory ret = _ownerExec(
            venue, abi.encodeCall(ICollateralVenue.repay, (address(usdc), type(uint256).max))
        );
    ...
    function test_D0_theThresholdIsInLoanTokenUnitsAndTwoOrdersAboveTheMeasuredError() public pure {
        assertEq(
            LoanDust.UNITS,
            100,
            "shared LOAN_DUST_UNITS; the agent's ABI seam pins the two together"
        );
    ...
    function test_D1_repayMaxHoldingExactlyTheBorrowRepaysEverythingHeldAndLeavesTheRoundingUnit()
        public
    {
    ...
    function test_D1c_anAccountHoldingNoUsdcIsRefusedByName() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                AaveV3Venue.InsufficientLoanToken.selector, address(usdc), 0, BORROW
            )
        );
        acct.execWithCallback(
            address(aaveVenue),
            0,
            abi.encodeCall(ICollateralVenue.repay, (address(usdc), type(uint256).max))
        );
        // a fixed amount above what is held is clamped the same way
        usdc.mint(address(acct), 400e6);
        bytes memory ret = _ownerExec(
            address(aaveVenue), abi.encodeCall(ICollateralVenue.repay, (address(usdc), 1_000e6))
        );
    ...
    function test_D2_aDustOnlyCurrentVenueDoesNotAttractTheWithdrawLeg() public {
        ...
        _ownerExec(
            address(morphoVenue),
            abi.encodeCall(ICollateralVenue.supply, (address(cbbtc), ONE_CBBTC))
        );
        _ownerExec(
            address(morphoVenue), abi.encodeCall(ICollateralVenue.borrow, (address(usdc), BORROW))
        );
        ...
        uint256 owedEverywhere = aaveVenue.debt(address(acct), address(usdc))
        ...
        u.swap =
            StrategyRouter.SwapQuote({quotedIn: 0, quotedOut: 0, maxSlippageBps: 0, routeData: ""});
        ...
        (, uint256 repaid, uint256 withdrawn,) =
            abi.decode(ret, (uint256, uint256, uint256, uint256));
        assertEq(
            repaid,
            owedEverywhere,
            "both books repaid: the rounding unit on Aave and the loan (with interest) on Morpho"
        );
