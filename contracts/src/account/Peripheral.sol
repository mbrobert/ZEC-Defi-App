...
///
/// @notice A peripheral's mutating functions are meant to be called BY an account (via `exec`,
///         `execAsKeeper`, or a nested peripheral call). `msg.sender` is that account; every token
