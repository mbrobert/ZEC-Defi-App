...
## 7. StrategyRouter  (`src/router/StrategyRouter.sol`) — stateless but for three per-account records (`entryHfWad` since A4, `solanaRecipient` since A5.1, `perpRecipient` since D5)
...
BurnToPerpParams { uint256 amount; uint256 maxFee; uint32 minFinalityThreshold; uint256 deadline }  (2026-09-26, Stream D / D5)
```
...
| `0xa63fc7ce` | `setPerpRecipient(address perpAccount)` | **2026-09-26, Stream D / D5** — the account's own `OilskinAccount` on HyperEVM as CCTP's `mintRecipient` for `burnToPerp` (a 20-byte address, left-padded on the wire), set by the owner through a plain `exec`; zero clears. When `PERP_FACTORY` / `PERP_ACCOUNT_IMPLEMENTATION` are set the address must be the one the SAME owner's account has there — `Clones.predictDeterministicAddress(impl, keccak256(abi.encode(owner)), factory)`, the factory's own rule — else `PerpRecipientMismatch(given, expected)`; both zero = unchecked (the owner's word, as the Solana recipient is) |
| `0x3be80aab` | `burnToPerp(BurnToPerpParams) → uint256 burned` | **2026-09-26, D5** — USDC the account holds → approve TokenMessengerV2 for exactly `burned` → `depositForBurn(burned, HYPEREVM_DOMAIN, bytes32(perpRecipient[account]), USDC, 0, maxFee, minFinalityThreshold)` → approve 0; `amount = max` = the whole balance; Fast (1000) is offered INTO HyperEVM at 1.3 bp, Standard (2000) otherwise — the caller reads Circle's schedule; `CrossChainDisabled` without a messenger OR with `HYPEREVM_DOMAIN` 0, `NoPerpRecipient(account)`, `MaxFeeNotBelowAmount`, `MaxFeeTooLarge` (1 %), `UsdcShort`, `ZeroAmount`, `Expired`; balance-delta assertion; a grant on this selector is bounded by its USDC budget and cannot redirect |
| views (2026-09-26) | `0x603aa73d` `HYPEREVM_DOMAIN()` · `0x651de557` `PERP_FACTORY()` · `0xa951eaeb` `PERP_ACCOUNT_IMPLEMENTATION()` · `0x9093b227` `perpRecipient(address) → address` | HyperEVM's domain 19 (`VERIFIED-PERPS-FACTS-2026-09-14.md` §7.4: `localDomain()` on chain 999 and Base's `remoteTokenMessengers(19)`), the HyperEVM factory and implementation (zero until that deployment exists), the per-account record. **Each rail is switched by its own domain**: `closeLpAndBurn` now also refuses `CrossChainDisabled` when `SOLANA_DOMAIN` is 0 |
