...
|---|---|---|
| Contracts (Foundry) | `cd contracts && forge test` | **469 passed / 0 failed / 13 skipped** (482 total), 42 suites (re-measured 2026-09-26: the perps rail's Base half +9, AUDIT-2026-09-26's regression file +5) |
...
| Root ABI seam | `node scripts/verify-abi.mjs` | **543** selectors / topics / errors across 20 contracts (2026-09-26: the router's perps pair and the venue's `OrderBelowMinimum`; 2026-09-25: `HyperliquidPerpVenue` added) |
| Shared | `npm test -w @zyo/shared` | **161** (2026-09-26: the venue's order rule; 2026-09-25: 19 perps tests) |
...
| Keeper | `npm test -w @zyo/agent` | **395 tests / 81 suites**, plus its own ABI seam **168 / 168** and the IDL seam **77 / 77** (2026-09-26: the perps path, +45, and the $10-minimum block, +2; the venue's fragment on the seam) |
...
| `StrategyRouterPerps.t.sol` | 6 | **2026-09-26, Stream D step D5** — the perps rail's Base half: `setPerpRecipient` unchecked (the fixture's router knows no HyperEVM factory: the owner's word, clearable, the account's not the wallet's) and checked (a router deployed with a stand-in HyperEVM factory accepts alice's derived address BEFORE it is deployed, refuses bob's, an EOA and alice's Base account with `PerpRecipientMismatch(given, expected)`, and the factory then deploys exactly there); `burnToPerp` Fast then "everything" Standard with the transmitter's message decoded as Circle attests it for HyperEVM — domain 6 → 19, to Base's own messenger address there, `destinationCaller` zero, the recipient left-padded, the sender the account — the supply falling, the approval reset, the router empty; every refusal by name (no record, zero, more than held, a fee not below the amount, a fee over 1 %, expired, Circle's denylist, "everything" of nothing); each rail switched by its own domain (Solana-only refuses `burnToPerp` while `closeLpAndBurn` works, and the reverse; no messenger refuses both) and both constructor guards; a keeper grant on `burnToPerp` bounded by its USDC budget and unable to redirect (no grant names `setPerpRecipient`) |
| `HyperliquidPerpVenue.t.sol` | 22 (20 + 2 on 2026-09-26, D5: the arrival from Base minting Circle's USDC into the account with no signature from it less the 1.3 bp executed fee, replay refused, `fundCore` moving it onto HyperCore spot and perp through the adapter with the approval reset; `burnToBase`'s message decoded as Circle attests it for Base — 19 → 6, Standard, anyone may deliver, the recorded Base account) | **2026-09-25, Stream D step D2** — the perps venue against CoreWriter's double and precompile doubles `vm.etch`ed at `0x…0800`–`0x…080f`: the ladder in Solidity equals shared `ladderBpsFor` for nine entries (11000 → 104931) rung for rung and disarm for disarm; the short's distance and equivalent HF on four scenes (the open, +10 %, +20 %, +40 %) and the sizing identities, every number produced by `perps.ts` first; action 1 / 7 / 13 encodings byte for byte with shared AND with the live `RawAction` log of 2026-09-25; `fundCore` approves exactly, deposits to spot and to perp dex 0, resets; `open` sends the documented IOC sell shaded by the band and records the entry, and refuses by name a position already open, another position on the account, a non-positive balance, an entry a unit under the floor, a notional over the cap, a reserve a wei short, a moved venue parameter, a mark 10 % off the oracle; `addMargin` / `reduce` / `withdrawToEvm` re-record the entry (D9) and keep the floor and the reserve; `burnToBase` burns Standard to the recorded Base account only; `protect` tops up at the repay rung, reduces at derisk, closes at emergency under the budgets, refuses the wrong keeper, a dead or stale-epoch grant, a forbidden rung, a rung not crossed, a reduce at the top-up rung, the owner path, no entry, no position, a second position; a precompile that does not answer stops both paths |
...
| `audit-regressions/PerpOrderPrice.t.sol` | 5 (incl. 1 fuzz) | **AUDIT-2026-09-26 P-1 (High) / P-2 (Low)**: the venue's price rule from its own page — "1234.5 is valid but 1234.56 is not", the sub-1 and szDecimals-1 examples, integers always allowed — as a predicate on `HyperCoreLib.roundOrderPxE8`; the four prices the D2 suite pinned on 2026-09-25 all INVALID (the defect) and their inside-the-band roundings (a sell's floor up, a buy's ceiling down); a fuzz over (price, szDecimals) that both roundings are valid, within one quantum and idempotent; the venue end to end — `open`'s sell and `reduce`'s buy carry valid prices inside the band, and a zero band rounds a sell's floor above the mark; `OrderBelowMinimum` by name for 0.01 ZEC at $800 on `open` and `reduce`, 0.02 accepted |
