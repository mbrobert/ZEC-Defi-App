...
| Root ABI seam | `node scripts/verify-abi.mjs` | **421** selectors / topics / errors across 19 contracts match (2026-09-11 W3-LOW-1: +`StrategyRouter.AmbiguousPositionId`; slice G: +`SlipstreamLpVenue.unstakedOverflow`; slice F: +`SlipstreamLpVenue`, +`SlipstreamPoolSwapAdapter`, `StrategyRouter.VenueWithdrawn` / `LP_VENUE_DIRECT` / `SWAP_DIRECT` / `UnknownPool` / `CollateralShort`, `ILpVenue.ownedPool`; 330 across 17 before) `contracts/abi/oilskin-abi.json`; `--write` regenerates; exit 1 on drift |
