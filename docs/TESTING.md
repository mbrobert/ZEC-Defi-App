...
checks derived from the block, levers re-derived. Two dated reads now sit side
by side on purpose and by rule: the market snapshot (the 2026-09-05 ledger
read — prices, 4.828 % borrow, 0.012 % / 1.843 % supply — in `web/lib/demo.ts`
`DEMO_MARKET` and the prototypes' `OIL_CHAIN_READ`) and the gate (the
2026-09-12 model). They may differ only when the gate is the fresher read,
both carry their dates, and the tests say so; refreshing the ledger read is a
chain-read task, not a model run, and was not part of this slice.
