...
|---|---|---|
| Contracts (Foundry) | `cd contracts && forge test --no-isolate` | **276 passed, 0 failed, 8 skipped** (284 total), 20 suites (2026-09-07, after `MorphoBlueVenue` was built: +22 `MorphoBlueVenueTest`, +11 `MorphoEntryFloor.t.sol`, −3 skeleton tests replaced; the row read 246 / 254 / 19 before that) — 6 invariants (+ a call-summary and a handler-liveness test) at 256 runs × depth 40; 11 fuzz tests × 512 runs. Measured on the founder's Mac, Foundry 1.8.1, 2026-09-07. **`--no-isolate` is required**: with isolation on, `PythOracleAdapter` splits `refresh()` and `price()` into two transactions and fails `NoUpdateInTx`. The figure this row carried before (244 / 252 / 18 suites) never matched a run on this machine, where the same tree measured **238 passed / 8 skipped**; the count is now what the command actually prints |
...
| Root ABI seam | `node scripts/verify-abi.mjs` | **321** selectors / topics / errors across 17 contracts match `contracts/abi/oilskin-abi.json`; `--write` regenerates; exit 1 on drift |
