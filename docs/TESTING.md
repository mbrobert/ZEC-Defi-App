...
| Keeper | `npm test -w @zyo/agent` | tsc + its own `verify-abi` **72/72** (pins `CollateralRegistry`, `AaveV3Venue`, the `ICollateralVenue` fragments against the interface and `MorphoBlueVenue`, and since 2026-09-09 `StrategyRouter.VenueRepaid`) + **221 tests / 45 suites** (~30 s; 2026-09-09: +6 `dispatcher.test.ts` "RISKS §8 residual (a)" on top of the venue-aware reader's +23 `venueReader.test.ts`, `venueGuard.test.ts` 6 → 8) |
