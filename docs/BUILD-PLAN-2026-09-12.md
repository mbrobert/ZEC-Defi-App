...
| B5 | Web: Solana wallet adapter, deposit/borrow flows, Kamino rate impact in the forecast | **built, not yet driven by a live wallet** — part 1 `67307df`: the yield service's Kamino reader and `/v1/solana/borrow` (the rate after the borrow shown, safety-only refusals — architecture §7 as built); part 2 (2026-09-13): wallet adapter, the five-screen wizard on Kamino's numbers with the cap named, the position page with the exit hatch, instructions pinned to the IDL (§8 as built). Remaining: a wallet-driven run on localnet and the cross-chain forecast (Stream C); **Advanced mode's sell budget of zero built 2026-09-20** (the review step's choice, the position card, the keeper's refusal by name; web unit 206, e2e 22 / 6, keeper 319) | Playwright scenarios (2 in demo mode pass; the signed path is next) |
