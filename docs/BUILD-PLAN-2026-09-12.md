...
D2 and D4 are built (2026-09-25 / 2026-09-26); nothing is deployed. The facts are read and the architecture
is decided; the order below is the one the Solana module proved works — facts, then design, then the program,
then the keeper, then the web, then an audit pass.
...
| D0b | The facts still taken from a document (the design's §2 widened the gate to seven) | **four of seven closed READ-ONLY 2026-09-25** (facts §7, raw in `docs/research/hyperevm-reads-2026-09-25.json`): the `position` struct decoded from `0x…0800` against a live short and the API at the same second (item 2); USDC's two HyperEVM contracts — Circle's `0xb883…630f` and HyperCore's linked adapter `0x6b9e…0a24` with its `deposit(uint256,uint32)` path, the CoreWriter encoding rule and action 13's tuple, all from one live transaction — and the system address (item 4's addresses); Circle's CCTP V2 contracts on chain 999 with `localDomain() = 19` and the fees (item 5); the margin table (§6). The venue's own liquidation rule reproduced its `liquidationPx` for every sampled short (design §4). **Open, testnet only:** action 1's bytes, action 7's `ntl` unit, the adapter's spot dex, a fresh account's leverage, a contract's first credit, a liquidation's residue — `scripts/perps-d0b-testnet.mjs`, written 2026-09-25 for the founder's throwaway key on chain 998, not yet run | proven against testnet, not read from a page |
...
| D2 | The account on HyperEVM: `OilskinAccount` UNCHANGED (the same clone factory, owner, `exec`, `execAsKeeper`, grants), with `HyperliquidPerpVenue` as its one peripheral — the position owned by the account's own address on HyperCore, the entry record (D9), the `PerpGrant` | **built 2026-09-25** — `contracts/src/venues/HyperliquidPerpVenue.sol` (`fundCore` / `open` / `addMargin` / `reduce` / `withdrawToEvm` / `burnToBase` owner path; `protect(rung, topUp, reduceSz)` keeper path inside a `PerpGrant`: rungs allowed, top-up and reduce budgets per period, slippage, tied to the account's `grantEpoch`), `PerpHealthLib.sol` (the shared ladder in Solidity value for value with `ladderBpsFor`; the short's distance and equivalent HF), `HyperCoreLib.sol` (the only code that knows the byte layouts; a read that does not decode reverts by name); `packages/shared/src/perps.ts` the TypeScript twin (19 tests); the ABI seam regenerated (20 contracts, 532 entries). Finding for §10 item 4: a +10 % move in one tick needs 1.38× the 1× reserve — the venue refuses the shortfall by name | **20 Foundry tests** against `MockCoreWriter` and `vm.etch`ed precompile doubles (a fork cannot run the precompiles) + testnet (D3) |
| D3 | The adapter against testnet (chain 998): first the EOA half — `scripts/perps-d0b-testnet.mjs` (one order, action 7, action 13, the adapter's `deposit`, a contract's first credit) — then the venue deployed there and driven once through an account | **script written 2026-09-25, not run; its order prices corrected 2026-09-26** (AUDIT-2026-09-26 P-1: the unrounded price would have been rejected and the run would have read "did not fill" without the reason) — rule 1: the first run is the founder's, with a throwaway key and testnet USDC; its JSON is the primary source the facts file's next addendum quotes | the D0b bytes accepted live; the entry record and one `protect` observed through the precompiles |
| D4 | The keeper's perps path: funding accrual, the short's own liquidation distance, the rungs that act on it | **built 2026-09-26** — `agent/src/perps/` (config, reader, valuation, policy, dispatcher, monitor, keeper; `npm run dev:perps -w @zyo/agent`). The reader makes the seven precompile reads `protect` makes, by `eth_call` at ONE block, plus the venue's entry record and the venue's own API as the independent mark; the valuation is fail-closed by name (P0 a read that did not decode, P1 a moved venue parameter, P2 mark against oracle beyond the venue's bound, P3 the API's mark against the precompile's, P4 a long or an isolated position, P5 a second position on the account — each UNKNOWN; a wiped account value is distance 0, HF 1.00, every rung fires). Each account's ladder derives from `entryOf` through shared `perpLadderFor` (D9). The plan: the reserve first at every rung (a reduce un-hedges, design §9 item 6), then the smallest reduce that reaches the disarm level plus the margin, capped at a third of the short and the grant's budgets, the whole short at the close rung, every reduce sized for the worst fill the grant's band allows; every clamp named. The dispatcher simulates `execAsKeeper([protect])` and classifies the venue's own error names (permanent / superseding / transient / failed), persists the nonce AND the intent (what was asked, the size and reserve it was asked against) before the broadcast, and `confirm` waits `PERPS_ACTION_DELAY_BLOCKS` after the receipt and judges the size and the spot reserve against that intent — an unfilled IOC is CONFIRMED and said, and the monitor re-arms and re-plans it, bounded per rung with the owner told once at the cap (the close rung never gives up). Funding accrual needs no module of its own: it moves the account value the distance is read from. Observe-only without `KEEPER_PERPS_PRIVATE_KEY`. Two findings for D7 (`RISKS.md` §23): an IOC that does not fill still charges the grant's reduce budget on chain; a third as the de-risk fraction is a keeper knob, not a venue rule | **45 keeper tests** on the Foundry scene's numbers (distance 4350 bps / HF 1.7699 at the open, the sizing numbers 67 and 104) over a behavioural HyperEVM fake driven through real viem — keeper 348 → **393** / 80 suites, the keeper's ABI seam 123 → **167** with the venue's fragment and the `protect` selector pinned — + testnet (D3) |
| D5 | USDC in and out over **CCTP** from the user's Base account — the rail Stream C already built | **the Base half and the arrival built 2026-09-26** (design §6 "as built") — `StrategyRouter.setPerpRecipient(address)` records the account's own HyperEVM account and, when the router was deployed knowing the HyperEVM factory and implementation (`PERP_FACTORY` / `PERP_ACCOUNT_IMPLEMENTATION`), refuses by name any address but the one the SAME owner's account has there (the factory's own CREATE2 rule; Base needs no HyperEVM state); `burnToPerp(amount, maxFee, minFinalityThreshold, deadline)` burns the account's USDC to `HYPEREVM_DOMAIN` (19, facts §7.4) with that record left-padded, anyone may deliver, the fee bounded, Fast or Standard as the caller asks; each rail switched by its own domain; `Deploy.s.sol` reads `CCTP_DOMAIN_HYPEREVM` / `PERP_FACTORY` / `PERP_ACCOUNT_IMPLEMENTATION` and requires Base's messenger to know the route. On HyperEVM the arrival is a plain `receiveMessage` (proven in Foundry: minted to the account with no signature from it, then `fundCore`) and the way home is D2's `burnToBase` (its message decoded as Circle attests it: 19 → 6, Standard). The ABI bundle regenerated (542 entries) and the web's generated file re-synced. **Not done, the founder's:** one crossing each way on Base Sepolia ↔ HyperEVM testnet with a throwaway key | **6 Foundry tests** in `StrategyRouterPerps.t.sol` + 2 in `HyperliquidPerpVenue.t.sol` + 1 in `Deploy.t.sol` (contracts 455 → **464** / 41 suites) + end to end on testnet |
...
| D7 | Internal audit pass over D2–D6 | **first wave 2026-09-26 over D2 / D4 / D5** — `docs/AUDIT-2026-09-26.md`: **P-1 High**, every order the venue sent carried a price HyperCore rejects (the precompile's eight-figure mark shaded by the band; the venue's rule is five significant figures and 6 − szDecimals decimals, read from its own page) — no open, reduce or keeper reduce could ever have filled on the real venue; fixed by `HyperCoreLib.roundOrderPxE8` rounding INSIDE the band, the shared twin, the D0b script; **P-2 Low**, an order under the venue's $10 minimum was sent into silence — the venue refuses `OrderBelowMinimum` by name, the keeper's plan lifts or drops a reduce and says why; the two D4 findings re-read (the budget charged on the receipt; the fraction) and left for the founder with D6; five observations. **The pass over D6 is still owed** | `docs/AUDIT-2026-09-26.md`; `contracts/test/audit-regressions/PerpOrderPrice.t.sol` (5); D6's pass before the freeze |
