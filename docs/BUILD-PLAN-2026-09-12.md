...
| B3 | Anchor program: init, deposit ZEC → Kamino, borrow USDC, repay, withdraw, delegated rungs, reserve, `depositForBurn` | **in progress** — scaffold `915350e`; owner path S2 `b576779` (localnet 15/15); `keeper_protect` S3 `ee291b9` (21/21). **Built:** `init_account`, `deposit`, `borrow` (entry floor), `repay`, `withdraw` (exit floor + dust), `transfer_out`, `close_position`, `grant` / `revoke` / `revoke_all`, `keeper_protect`. **Designed 2026-09-13, not yet built** (`SOLANA-ARCHITECTURE.md` §14): the entry-HF record and the per-position ladder on chain (D7 parity — until it lands the program still judges every account against the floor's ladder, which refuses the keeper at the derived rungs), the reserve rule, `set_base_account`, `deposit_for_burn` (the CCTP CPI). **Cannot be built:** `release_obligation` (the S3 finding, below) | local-validator tests with cloned Kamino + CCTP programs |
...
| B5 | Web: Solana wallet adapter, deposit/borrow flows, Kamino rate impact in the forecast | **built, not yet driven by a live wallet** — part 1 `67307df`: the yield service's Kamino reader and `/v1/solana/borrow` (the rate after the borrow shown, safety-only refusals — architecture §7 as built); part 2 (2026-09-13): wallet adapter, the five-screen wizard on Kamino's numbers with the cap named, the position page with the exit hatch, instructions pinned to the IDL (§8 as built). Remaining: a wallet-driven run on localnet, Advanced mode's sell budget of zero, the cross-chain forecast (Stream C) | Playwright scenarios (2 in demo mode pass; the signed path is next) |
