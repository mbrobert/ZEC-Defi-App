...
| A3 | **Gate → forecast**: `/v1/forecast`; remove `ok`-blocking in `web/` and `prototype/`; acknowledgment; liquidity hard-refusal; rewrite README, RISKS §14, the prototype strings | **not started** — the gate still refuses (slice K `d2f7760` re-ran it on fresh reads: nothing clears at any borrow rate); the §6 phrases are present in README, RISKS §14, `web/lib/gate.ts`, `StrategyStep.tsx`, both prototypes and their suites, `services/yield/src/gate.ts` | 0 stale references; copy test green |
| A4 | **Risk slider** (§2b): one registry floor; `ladderFor(entryHf)`; HF ↔ borrow both ways; presets as marks; entry HF stored on the position and read by the keeper; acknowledgment below 1.55 | **not started** — the tree carries the fixed 1.55 floor (`CollateralRegistry`, shared `HF_LADDER`, `solana/…/generated/ladder.rs` pinned by the seam) and both keepers read the global table. The floor number waits on the founder (1.25 proposed) | fork test: a position opened at HF 1.30 warns at 1.27; one at 1.25 has emergency at 1.05 |
