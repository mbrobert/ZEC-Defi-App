...
| D3 | The adapter against testnet (chain 998): first the EOA half — `scripts/perps-d0b-testnet.mjs` (one order, action 7, action 13, the adapter's `deposit`, a contract's first credit) — then the venue deployed there and driven once through an account | **script written 2026-09-25, not run** — rule 1: the first run is the founder's, with a throwaway key and testnet USDC; its JSON is the primary source the facts file's next addendum quotes | the D0b bytes accepted live; the entry record and one `protect` observed through the precompiles |
