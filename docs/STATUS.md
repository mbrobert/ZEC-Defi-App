...
_Generated 2026-09-16T00:19:47Z by `npm run status` on darwin-arm64, Node 22.23.2._

**Tree:** the working tree at `fed6189` on `main`, with **9 file(s) modified on top of it** — including this one, when it is regenerated just before a commit. Each suite ran against that tree, not against a published commit.
...
| Shared | `npm test -w @zyo/shared` | **119** |
...
| Web, unit | `npm test -w @zyo/web` | **200** |
