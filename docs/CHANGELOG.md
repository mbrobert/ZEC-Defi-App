...
chain, and ROADMAP §3's fallback is that it ships behind a flag. So the wizard now shows the loop as the forecast
it is and says plainly what this build cannot yet sign.

- **A sixth screen, "Borrowed USDC"**, between the health factor and the review (`DeployStep`): keep the USDC on
  Solana (the default), or cross to the user's own Base account into an Aerodrome pool. The cells are the
  service's `/v1/forecast?crossChain=1` — priced by Kamino's rate after the borrow, the ZEC reserve's threshold
  and cap, the USDC pool's depth — Simple listing one cell per pool at its best setting, Advanced every pool ×
  setting with both models. The reserve of §14.3 is named on the screen as what stays on Solana whatever is
  chosen; a chosen pool shows what crosses, the user's net at this position's LTV, the two cross-chain
  disclosures, and the Base wizard's acknowledgment with the loop's own sentence. Only a safety refusal stops a
  cell (D4/D5).
- **The choice is gated** by `NEXT_PUBLIC_CROSS_CHAIN_LOOP`: off, the forecast is information and the option says
  the crossing is not in this build; on (the e2e server), a pool may be chosen, the review names both amounts,
  and the sign screen lists §14.6's four crossing signatures — record the Base account, burn to Base, receive and
  open, the cross-chain grant — as **not signable in this build**. The USDC stays on Solana until they exist.
- **The service's Kamino-side reader is one pure function** (`venueBorrowFromKamino`, `services/yield/src/solanaBorrow.ts`),
  extracted from the server so the demo generator can run it on the recorded capture:
  `gen-demo-forecast.mjs` now also writes `samples/demo-forecast-crosschain.json` (27 cells, mirrored into
  `web/lib/solana/`), pinned by `test/demo-forecast-crosschain.test.ts`. The Base snapshot regenerated byte-identical.
- Tests: yield **192 → 194**, web unit **206 → 211**, e2e **22 → 24** (6 skipped); the web's type check clean.
- **Backlog T-1 closed** by its own rule: twenty consecutive `npm run status` runs, twenty green prototype rows,
  the two non-zero exits in the series being the web unit suite against this entry's test mid-edit — named as
  such by the kept log. The 1,069 s hang was never reproduced under the instrumented harness; if it returns, the
  harness says which check it stopped after.
