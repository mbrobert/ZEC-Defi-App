...
| `interfaces/ICctpV2.sol` | 71 | `ITokenMessengerV2` / `IMessageTransmitterV2` — Circle's CCTP V2 as transcribed from the VERIFIED implementations behind the Base proxies (`VERIFIED-SOLANA-FACTS.md` Addendum 3, 2026-09-13); every selector pinned by `verify-abi` | — |
| `router/StrategyRouter.sol` | 1000 | `openLeveragedLp`, `openBorrowOnly`, `unwind`, `sweep`, and since 2026-09-13 (D6 / A5.1) `openLpOnly` (USDC already held → LP, no borrow), `setSolanaRecipient` (the owner's one destination for a burn) and `closeLpAndBurn` (close → swap → CCTP `depositForBurn` to that destination, the approve charged to a keeper grant's USDC budget); stateless but for the two per-account records (`entryHfWad`, `solanaRecipient`); asserts its balance of every token it touches is **unchanged** (a delta, not a zero). Since 2026-09-11: two LP venues (engine first, then direct — by pool id on open, by `ownedPool` on unwind) and the withdraw leg visiting every venue holding the account's collateral, one `VenueWithdrawn` each | none |
