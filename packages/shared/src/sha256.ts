/**
 * Pure-TypeScript SHA-256 (FIPS 180-4). Zero dependencies, no node:crypto —
 * @zyo/shared is environment-agnostic (browser + node) by design.
 *
 * Used for Base58Check validation of Zcash transparent addresses
 * (double-SHA256 checksum). Verified against the canonical NIST vectors
 * (empty string, "abc", "abcdbcde…", 1M × "a") in the agent test suite
 * (agent/test/zcashAddress.test.ts).
 */
...
/** Convenience: SHA-256 over a UTF-8 string, hex-encoded (test vectors). */
